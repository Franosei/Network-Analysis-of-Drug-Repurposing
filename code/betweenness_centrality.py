...
# Optional: Reduce size for performance (top 300 degree)
top_k = 100
...
top_n = 20
top_drugs = sorted(betweenness.items(), key=lambda x: x[1], reverse=True)[:top_n]
...
# Layout (faster than spring_layout for large graphs)
...
plt.title("Drug Network – Top 20 by Betweenness Centrality", fontsize=16)
plt.axis("off")
plt.tight_layout()
plt.show()
